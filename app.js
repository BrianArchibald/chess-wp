...
function executeTapMove(targetSquare) {
    ...
    // 10ms timeout prevents browser touch bugs when capturing an enemy piece
    setTimeout(() => {
...
function onDragStart (source, piece, position, orientation) {
    ...
    // 1. If you touch an ENEMY piece while you have a square selected...
    if ((game.turn() === 'w' && piece.search(/^b/) !== -1) ||
        (game.turn() === 'b' && piece.search(/^w/) !== -1)) {

        if (selectedSquare) {
            executeTapMove(source); // Execute the capture!
        }
        return false; // Prevent dragging an enemy piece
    }

    // 2. If you touch a NEW piece of your own color, clear the old highlight
...
function onDrop (source, target) {
    // 3. If you pick up a piece and drop it on the SAME square, it's a Tap!
    if (source === target) {
    ...
    // 4. Otherwise, process the move as a normal drag
    let result = processPlayerMove(source, target);
    selectedSquare = null;
    return result;
}

// 5. If you tap an EMPTY square, chessboard.js ignores it. This catches it!
$('#board').on('mousedown touchstart', '.square-55d63', function(e) {
    ...
    // If there is NO piece here, and we have a piece selected, move it!
